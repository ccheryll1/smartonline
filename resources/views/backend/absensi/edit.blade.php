...
@section('content')
    ...
    <form action="{{ route('absensi.update', $absensi->id) }}" method="POST" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        ...
        <div class="mb-4">
            ...
            <input type="time" name="clock_in" id="clock_in" value="{{ $absensi->clock_in }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring" required>
        </div>
        <div class="mb-4">
            <label class="block text-gray-700 text-sm font-bold mb-2" for="clock_out">Clock Out</label>
            <input type="time" name="clock_out" id="clock_out" value="{{ $absensi->clock_out }}" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:ring" required>
        </div>
        <div class="mb-4">
